...
include './modules/DESeq2.nf' params(__params)
include DESeq2 as DESeq2ForIso from './modules/DESeq2.nf' params(__params)
include DESeq2 as DESeq2ForGene from './modules/DESeq2.nf' params(__params)
...
workflow RNAseq_paired {
   ...
   main:
     ...
     DESeq2ForIso(count_iso)
     DESeq2ForGene(count_gene)
...
workflow RNAseq_single {
   ...
   main:
     ...
     report_qc2 = FastqcForSingleAfter(reads_trimed)
     qc1 = report_qc1.map {[it[1]]}.flatten().toList()
     qc2 = report_qc2.map {[it[1]]}.flatten().toList()
     combined_qc = qc1+qc2
...
workflow {
  ...
  if (__params.mode == "fastq"){
      if (__params.reads_type == "pair-end" ) {
          reads = Channel.fromFilePairs("${__params.input}/*_R{1,2}*.fastq.gz")
  ...
  else if (__params.mode == "csv") {
     fastq = "${__params.fastq}"
     csv_ch = Channel.fromPath("${__params.input}")
     reads_csv = csv_ch.splitCsv(header:true).map { row -> def sampleID = row.sampleID.trim()

     if (row.read2) {
        tuple(sampleID, file("${fastq}/${row.read1.trim()}"), file("${fastq}/${row.read2.trim()}"))
     }
     else {
        tuple(sampleID, file("${fastq}/${row.read1.trim()}"))
     }
     }
     reads_csv.view()

     paired_ch = reads_csv.filter { it.size() == 3 }
     single_ch = reads_csv.filter { it.size() == 2 }
    
     if (__params.reads_type == "single-end" ) {
          RNAseq_single(single_ch, fasta, gtf)
     }
     else if (__params.reads_type == "pair-end") {
          RNAseq_paired(paired_ch, fasta, gtf)
     }
  }    
