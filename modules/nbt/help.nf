...
def printArg(workflowHomePath, field) {
    def resolvePath = pathResolver(workflowHomePath, params.paramsPath)
    if (field.type instanceof List) {
      def enumArg = field.type.tail().find({ it instanceof Map && it.type == "enum" })
      if (enumArg) {
        log.info "|   --${field.name}  <${enumArg.symbols.join("|")}>     ${field.doc}"
      ...
      if (field.type.type == 'enum') {
        log.info "|   --${field.name}  <${field.type.symbols.join("|")}>     ${field.doc}"
...
def showOptionsArguments(schema) {
  ...
  optionalArgs.each { printArg(workflowHomePath, it) }
	log.info "|   --export-key-schema                   Print key.avsc schema."
	log.info "|   --export-value-schema                 Print value.avsc schema."
	log.info "|--------------------------------------------------------------------------------------------------"
}
